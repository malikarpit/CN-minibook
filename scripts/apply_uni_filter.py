...
MAPPINGS = {
    ...
    "chapters/ch04-devices-sdn.html": {
        "s-repeater": (True, "Network Devices (Repeater — Layer 1 Regeneration)"),
        ...
        "s-firewall": (False, "Firewall & Security Filtering (Supplementary Deep-Dive)"),
    ...
    "chapters/ch06-media-signaling.html": {
        ...
        "s-fiber": (True, "Physical layer: transmission media (Optical Fiber Cable: Physics & Total Internal Reflection)"),
        "s-radio": (True, "Physical layer: transmission media (Unguided Media: Radio Waves 3 kHz to 1 GHz)"),
        "s-microwave": (True, "Physical layer: transmission media (Unguided Media: Microwaves 1 GHz to 300 GHz)"),
        "s-infrared": (True, "Physical layer: transmission media (Unguided Media: Infrared Waves 300 GHz to 400 THz)"),
        "s-satellite": (False, "Satellite Communication GEO/MEO/LEO (Supplementary Deep-Dive)"),
        ...
        "s-line-compare": (True, "Physical layer: signaling (Line Coding Schemes Comparison)"),
    ...
    "chapters/ch07-modulation-switching.html": {
        ...
        "s-multiplex-intro": (False, "Multiplexing Fundamentals (Supplementary Deep-Dive)"),
        "s-fdm": (False, "Frequency Division Multiplexing FDM (Supplementary Deep-Dive)"),
        "s-tdm": (False, "Time Division Multiplexing TDM (Supplementary Deep-Dive)"),
        "s-wdm": (False, "Wavelength Division Multiplexing WDM (Supplementary Deep-Dive)"),
        "s-mux-compare": (False, "Multiplexing Techniques Comparison (Supplementary Deep-Dive)"),
        ...
        "s-message-switching": (False, "Message Switching — Historical Context (Supplementary Deep-Dive)"),
    ...
    "chapters/ch10-mac-ethernet.html": {
        ...
        "s-controlled-access": (False, "Controlled Access Protocols: Reservation, Polling, Token (Supplementary Deep-Dive)"),
        "s-channelization": (False, "Channelization: FDMA, TDMA, CDMA (Supplementary Deep-Dive)"),
  ...
  <div class="uni-mode-toggle-card">
    <div>
      ...
      <div style="font-size:0.8rem; color:var(--tx-muted); margin-top:0.2rem;">
        Focus specifically on sections prescribed in Delhi University syllabus vs supplementary deep-dives.
      </div>
    </div>
...
def process_file(filepath, sec_dict):
    ...
    for sec_id, (is_uni, topic_name) in sec_dict.items():
        ...
        def replace_toc(m):
            ...
            if is_uni:
                ...
                classes.append('not-in-uni')
                tag = '<span class="toc-supp-tag">🔬 Extra</span>'
    ...
    # 3. Update sections (class and banner)
    for sec_id, (is_uni, topic_name) in sec_dict.items():
        sec_class_to_add = "section-in-uni" if is_uni else "section-not-in-uni"
        
        # Update <section ... id="sec_id" ...> class
        sec_open_re = rf'(<section\b[^>]*\bid=[\"\']{sec_id}[\"\'][^>]*>)'
        m = re.search(sec_open_re, content)
        if m:
            open_tag = m.group(1)
            ...
            if 'class="' in open_tag_new:
                open_tag_new = open_tag_new.replace('class="', f'class="{sec_class_to_add} ')
            elif "class='" in open_tag_new:
                open_tag_new = open_tag_new.replace("class='", f"class='{sec_class_to_add} ")
            else:
                open_tag_new = open_tag_new[:-1] + f' class="{sec_class_to_add}">'
                
            content = content[:m.start()] + open_tag_new + content[m.end():]

        # Insert banner after header container
        sec_start = content.find(f'id="{sec_id}"')
        if sec_start == -1:
            sec_start = content.find(f"id='{sec_id}'")
            
        if sec_start != -1:
            sec_end_scope = min(sec_start + 2500, len(content))
            chunk = content[sec_start:sec_end_scope]
            
            if 'uni-req-banner' not in chunk and 'non-uni-req-banner' not in chunk:
                if is_uni:
                    banner = f"""\n  <div class="uni-req-banner">\n    <span class="uni-req-badge">🎓 Uni Required</span>\n    <span>Required for syllabus topic: <strong>{topic_name}</strong></span>\n  </div>\n"""
                else:
                    banner = f"""\n  <div class="non-uni-req-banner">\n    <span class="uni-req-badge" style="background:rgba(148,163,184,0.18); border-color:rgba(148,163,184,0.4); color:#94a3b8;">🔬 Supplementary</span>\n    <span>Supplementary deep-dive / GATE detail — not directly in standard university syllabus</span>\n  </div>\n"""
                
                # Find closing </h2> and then the next </div>
                h2_idx = chunk.find('</h2>')
                if h2_idx != -1:
                    close_div = chunk.find('</div>', h2_idx)
                    if close_div != -1:
                        insert_pos = sec_start + close_div + len('</div>')
                        content = content[:insert_pos] + banner + content[insert_pos:]
                    else:
                        insert_pos = sec_start + h2_idx + len('</h2>')
                        content = content[:insert_pos] + banner + content[insert_pos:]
...
def main():
    for filepath, sec_dict in MAPPINGS.items():
        if os.path.exists(filepath):
            process_file(filepath, sec_dict)
        else:
